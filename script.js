...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const tooltipSelectors = [
        {
            ...
            elements: [
                ...
                ".tooltip:nth-child(1) .description p .line > span",
            ],
        },
        {
            trigger: 0.85,
            elements: [
                ...
                ".tooltip:nth-child(2) .description p .line > span",
            ],
        },
    ];
    ...
    function setupModel() {
        ...
        model.position.set(
            isMobile ? center.x + modelSize.x * 0.1 : center.x - modelSize.x * 0.4,
    ...
    let mixer, action, clipDuration;
    new GLTFLoader().load("/models/gift_loot_box_thing_wip.glb", (gltf) => {
        model = gltf.scene;
        mixer = new THREE.AnimationMixer(model);
        const clip = gltf.animations[0]; // or find by name
        action = mixer.clipAction(clip);

        action.play();
        action.paused = true; // don't let it auto-play, we'll drive it manually
    ...
    ScrollTrigger.create({
        ...
        scrub: 1, // smoothing — try 0 for exact 1:1, or a number for lag/ease
        onUpdate: (self) => {
            if (!mixer || !action) return;
            action.time = self.progress * clipDuration;
            mixer.update(0); // force mixer to apply the new time without advancing it
        }
    });
    function animate() {
    ...
    ScrollTrigger.create({
        ...
        onUpdate: ({ progress }) => {
            ...
            const scaleX =
                progress < 0.45 ? 0 : progress > 0.65 ? 100
